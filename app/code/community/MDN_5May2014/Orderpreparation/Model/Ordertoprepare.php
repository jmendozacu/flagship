...
class MDN_Orderpreparation_Model_OrderToPrepare extends Mage_Core_Model_Abstract {
            ...
            if ($remaining_qty > 0) {
				...
				if ($item->getproduct_type() == 'configurable')
				{
					foreach ($order->getItemsCollection() as $subItem)
					{
						...
						{
							$subProductRemainingQty = ($subItem->getqty_ordered() - $subItem->getRealShippedQty());
    ...
    public function CanAddOrder($orderId) {
        ...
        foreach ($order->getItemsCollection() as $item) {
            ...
            if ($ManageStock) {
                if (($remaining_qty > 0)) {
                    //recupere la qte de ce produit d�ja ajout� dans la pr�paration de commande
    ...
    public function getPendingOrdersIds() {
        ...
        $collection = Mage::getResourceModel('sales/order_collection')
						...
						//->getAllIds() <-- commented because raise error on magento 1.4.1.1
						;
