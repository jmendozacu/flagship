...
class MDN_Orderpreparation_Block_Widget_Tab_OrderPreparationTab extends Mage_Adminhtml_Block_Widget_Tabs {
    ...
    protected function _toHtml() {
        ...
        $forceRefreshUrl = $this->getUrl('OrderPreparation/OrderPreparation/RefreshList');
        ...
        if (Mage::getSingleton('admin/session')->isAllowed('admin/erp/order_preparation/prepare_order/allow_scope_change')) {
            ...
            $button .= ' <b>' . $this->__('Operator : ') . '</b>' . $this->getOperatorCombo();
            $button .= '</div>';
            $button .= '<div align="right" style="width: 50%; float: left;">';
            $button .= '<button onclick="document.location.href=\'' . $forceRefreshUrl . '\'" class="scalable save" type="button"><span>' . $this->__('Force Refresh') . '</span></button>';
            $button .= '</div>';
            ...
            //declare url
            $script = '<script>';
            $script .= "urlChangePreparationWarehouse = '" . $this->getUrl('OrderPreparation/OrderPreparation/setPreparationWarehouse') . "';";
            $script .= "urlChangeOperator = '" . $this->getUrl('OrderPreparation/OrderPreparation/setOperator') . "';";
            $script .= '</script>';
        }
