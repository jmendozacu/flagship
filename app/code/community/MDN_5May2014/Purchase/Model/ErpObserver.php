...
class MDN_Purchase_Model_ErpObserver {
        ...
        if (mage::helper('purchase/Product_Packaging')->isEnabled()) {
            ...
            if ($packagingData['new']['pp_name'] != '') {
                $newPackaging = mage::getModel('Purchase/Packaging')
                                ...
                                ->setpp_is_default($packagingData['new']['pp_is_default'])
                                ->setpp_is_default_sales($packagingData['new']['pp_is_default_sales'])
                                ->save();
            }
    ...
    public function advancedstock_product_aftersave(Varien_Event_Observer $observer) {
        ...
        //if manual supply need qty , excluyde from supply needs or waiting for delivery qty changes : update supply needs
        if (($manualSupplyNeedQtyAfterSave != $manualSupplyNeedQtyBeforeSave)
                || ($excludeFromSupplyNeedAfterSave != $excludeFromSupplyNeedBeforeSave)) {
    ...
    public function advancedstock_stock_aftersave(Varien_Event_Observer $observer) {
        ...
        //if changes, update supply needs
        if (($qtyAfterSave != $qtyBeforeSave)
                || ($stockOrderedQtyAfterSave != $stockOrderedQtyBeforeSave)
                || ($stockOrderedQtyForValidOrdersAfterSave != $stockOrderedQtyForValidOrdersBeforeSave)
                || ($manageStockAfterSave != $manageStockBeforeSave)
                || ($idealStockAfterSave != $idealStockBeforeSave)
                || ($notifyQtyAfterSave != $notifyQtyBeforeSave)) {
            //plan update supply needs
