		...
		if ($this->_lateOrders == null)
		{
			...
			if (mage::helper('AdvancedStock/FlatOrder')->ordersUseEavModel())
			{
				$this->_lateOrders = mage::getModel('sales/order')
				        			        ...
				        			        ->joinTable(mage::getModel('Purchase/Constant')->getTablePrefix().'purchase_sales_order_planning',
											        	'psop_order_id=entity_id',
														array(
			...
			else 
			{
				$this->_lateOrders = mage::getModel('sales/order')
											...
											->join('SalesOrderPlanning/Planning', 'psop_order_id=entity_id and DATEDIFF(psop_delivery_date, anounced_date) > 0')
