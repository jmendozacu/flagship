...
class MDN_SalesOrderPlanning_Block_Planning_Cart extends Mage_Checkout_Block_Cart_Abstract
{
	...
	public function getDeliveryMsg()
	{
		...
		//define message
		$retour = '<div>';
		$retour .= mage::helper('SalesOrderPlanning')->__('Your order should be delivered on <b>%s</b>', $deliveryDate);
		$retour .= '<br>'.mage::helper('SalesOrderPlanning')->__('At worst, we commit you deliver on <b>%s</b>', $deliveryMaxDate);
		if (false)
