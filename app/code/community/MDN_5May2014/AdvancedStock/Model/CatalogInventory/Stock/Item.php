...
class MDN_AdvancedStock_Model_CatalogInventory_Stock_Item extends Mage_CatalogInventory_Model_Stock_Item {
    ...
    public function loadByProductWarehouse($productId, $warehouseId) {
        $retour = null;
	//echo '___'.$productId.'_____'.$warehouseId.'___';
        $collection = $this->getCollection()
                        ->addFieldToFilter('product_id', trim($productId))
                        ->addFieldToFilter('stock_id', trim($warehouseId));
        //todo : use getFirstItem method
    ...
    public function checkQty($qty) {
        ...
        if ($this->getAvailableQtyForSale() - $qty < 0) {
    ...
    protected function _afterSave() {
        ...
        if ($this->getqty() != $this->getOrigData('qty')) {
            ...
            if ($qtyFromStockMovement != $this->getqty()) {
                ...
                mage::getModel('AdvancedStock/StockMovement')->createStockMovement($productId,
                        ...
                        $targetWarehouseId,
                        $diff,
                        mage::helper('AdvancedStock')->__('Adjustment'),
                        $additionalData);
            }
        }
        ...
        if ($qtyAfterSave < $qtyBeforeSave) {
            if ($reservedQty > $qtyAfterSave) {
                //todo: error, reserved qty CAN NOT BE GREATER THAN stock qty
            }
        }
        ...
        if ($this->getOrigData('is_in_stock') != $this->getis_in_stock())
        {
            ...
            mage::helper('BackgroundTask')->AddTask('Force product reindex for #'.$productId,
    ...
    {
        if (Mage::getStoreConfig('advancedstock/prefered_stock_level/enable_ideal_stock'))
        {
            ...
            $value = $this->getWarningStockLevel ();
        return (int)$value;
    }
    ...
    public function getAvailableQtyForSale()
    {
        $productId = $this->getProductId();
        $websiteId = 0;
        $stocks = mage::helper('AdvancedStock/Product_Base')->getStocksForWebsiteAssignment($websiteId,
                                                                                            MDN_AdvancedStock_Model_Assignment::_assignmentSales,
                                                                                            $productId);
        $stockLevel = 0;
        $pendingOrderQty = 0;
        foreach ($stocks as $stock)
    ...
    public function getNeededQty() {
        ...
        if ($this->getQty() <= $this->getWarningStockLevel())
        {
