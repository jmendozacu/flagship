...
class MDN_SalesOrderPlanning_Model_Planning  extends Mage_Core_Model_Abstract
{
			...
			if (Mage::getStoreConfig('planning/consider/consider_order_on_paypment_validated') == 1)
			{
				...
				if ($order->getpayment_validated() == 1)
				{
					...
					if (date('G', $considerationDateTimeStamp) >= $maxHour)
					{
						...
						$considerationComments .=  mage::helper('SalesOrderPlanning')->__('Order placed at %s h<br>', date('H', $considerationDateTimeStamp));
				}
				else 
				{
	...
	******************************************************************************************************************************/
	...
	{
		return mage::helper('SalesOrderPlanning/Holidays')->isHolyDay($dateTimestamp);
	}
    ...
    protected function _afterSave()
    {
	    	...
	    	//todo : improve update algorithm
	    	if ($UpdateSupplyNeeds)
	    	{
	    		$order = mage::getModel('sales/order')->load($this->getpsop_order_id());
	    		foreach($order->getAllItems() as $item)
				{
					$remainToShip = $item->getRemainToShipQty();
					if (($remainToShip > 0) && ($remainToShip > $item->getreserved_qty()))
					{
						//plan supply need update
						$productId = $item->getproduct_id();
