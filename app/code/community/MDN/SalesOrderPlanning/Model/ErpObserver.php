 ...
 */
//observerS to handle events that implies planning update
class MDN_SalesOrderPlanning_Model_ErpObserver {
    ...
    public function salesorder_aftersave(Varien_Event_Observer $observer) {
        ...
        //if just considered, plan task to create planning
        if (($order->getstocks_updated() != $order->getOrigData('stocks_updated'))) {
    ...
    public function product_delivery_date_change(Varien_Event_Observer $observer) {
        ...
        mage::helper('BackgroundTask')->AddTask('Update product availability status for product #' . $productId . ' (delivery date has changed)',
                'SalesOrderPlanning/ProductAvailabilityStatus',
                'RefreshForOneProduct',
                $productId,
                null,
    ...
    public function product_waiting_for_delivery_qty_change(Varien_Event_Observer $observer) {
        ...
        //update product availability status
        mage::helper('BackgroundTask')->AddTask('Update product availability status for product #' . $productId . ' (Waiting for delivery qty has changed)',
                'SalesOrderPlanning/ProductAvailabilityStatus',
                'RefreshForOneProduct',
                $productId,
                null,
    ...
    public function salesorderitem_aftersave(Varien_Event_Observer $observer) {
        ...
        if ($reservedQtyAfterSave != $reservedQtyBeforeSave) {
            ...
            mage::helper('BackgroundTask')->AddTask('Store real fullstock date for order #' . $orderId,
                    'SalesOrderPlanning/Planning',
                    'storeRealFullStockDate',
                    $orderId,
                    null,
                    true);
