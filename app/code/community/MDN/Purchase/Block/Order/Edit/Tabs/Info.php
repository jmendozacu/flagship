	...
	public function getNotices()
	{
		...
		if (mage::getStoreConfig('purchase/purchase_order/display_order_notices'))
		{
			...
			if (!$order->isFreeCarriageForAmount())
				$notices[] = ($this->__('Order total (%s) doesnt fullfill free carriage total condition (%s)', $order->getProductTotalBase(), $order->getSupplier()->getsup_free_carriage_amount()));
			if (!$order->reachOrderMinimumAmount())
				$notices[] = ($this->__('Order total (%s) doesnt fullfill minimum amount condition (%s)', $order->getProductTotalBase(), $order->getSupplier()->getsup_order_mini()));
		}
