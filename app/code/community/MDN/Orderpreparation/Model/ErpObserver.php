...
class MDN_Orderpreparation_Model_ErpObserver {
    ...
    public function salesorderitem_aftersave(Varien_Event_Observer $observer) {
        ...
        //dispatch order if warehouse change
        $warehouseAfterSave = $salesOrderItem->getpreparation_warehouse();
        $warehouseBeforeSave = $salesOrderItem->getOrigData('preparation_warehouse');
        if ($warehouseAfterSave != $warehouseBeforeSave) {
            $orderId = $salesOrderItem->getorder_id();
            //plan dispatch order
            $this->dispatchOrder($orderId);
            return true;
