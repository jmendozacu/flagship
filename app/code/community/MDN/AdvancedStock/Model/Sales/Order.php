...
class MDN_AdvancedStock_Model_Sales_Order extends Mage_Sales_Model_Order
{
    ...
    protected function _afterSave()
    {
    	...
    	if ($this->getis_valid() != $this->getOrigData('is_valid'))
    	{
    		...
    		{
    			mage::helper('AdvancedStock/Product_Base')->planUpdateStocksWithBackgroundTask($item->getproduct_id());
    		}
    	}
    	...
    	if ($this->getstatus() != $this->getOrigData('status'))
    	{
    		...
    		{
    			foreach ($this->getAllItems() as $product)
    			{
    				//set product reserved qty to 0
    				$product->setreserved_qty(0)->save();
    				
    				//plan product stocks update
					mage::helper('BackgroundTask')->AddTask('Update stocks for product #'.$product->getproduct_id(), 
    ...
    /**
     * Define if we can reserve products for this order
     *
     */
