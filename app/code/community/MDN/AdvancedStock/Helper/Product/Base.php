...
class MDN_AdvancedStock_Helper_Product_Base extends Mage_Core_Helper_Abstract {
    ...
    public function updateStocks($product) {
        ...
        foreach ($stocks as $stock) {
            ...
            if ($stock->ManageStock()) {
                ...
                //update reservation
                mage::helper('AdvancedStock/Product_Reservation')->reserveProductForPendingOrders($product->getId());

                //update reserved qty
                mage::helper('AdvancedStock/Product_Reservation')->storeReservedQtyForStock($stock, $product->getId());

                //update ordered qty
                mage::helper('AdvancedStock/Product_Ordered')->storeOrderedQtyForStock($stock, $product->getId());
            }
        }
    }
     ...
     * @param unknown_type $productId
     */
    public function planUpdateStocksWithBackgroundTask($productId) {
