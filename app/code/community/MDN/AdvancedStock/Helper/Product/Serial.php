...
class MDN_AdvancedStock_Helper_Product_Serial extends Mage_Core_Helper_Abstract {
	 ...
	 */
	public function addAVSerialsFromDelivery($pop_product_id, $purchaseOrder, $serials, $location = null) {
		//echo "running ".__CLASS__." addSerialsFromDelivery where product id = $pop_product_id, purchase order id = {$purchaseOrder->getId()}, serials: ".implode(', ', $serials).", and location $location<br>\n";
		if(!$location) {
			$location = "DOCK";
			//echo "location was empty; set to DOCK<br>\n";
		}
		
		foreach ($serials as $serial) {
			//echo "serial: $serial<br>\n";
			$serial = trim($serial);
			//echo "trimmed serial: '$serial'<br>\n";
			if ($serial == '')
				continue;
			
			//if not exist, insert
			if (!$this->serialExists($serial)) {
				//echo "creating AdvancedStock/ProductSerial $serial for purchase order id {$purchaseOrder->getId()} product id {$pop_product_id} with location = '$location'<br>\n";
				mage::getModel('AdvancedStock/ProductSerial')
						->setpps_product_id($pop_product_id)
						->setpps_purchaseorder_id($purchaseOrder->getId())
						->setpps_serial($serial)
						->setAvLocation($location)
						->save()
				;
			}
		}
