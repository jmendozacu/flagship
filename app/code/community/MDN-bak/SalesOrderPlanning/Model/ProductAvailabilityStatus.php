...
class MDN_SalesOrderPlanning_Model_ProductAvailabilityStatus extends Mage_Core_Model_Abstract {
    ...
    protected function getAvailableQty() {
        $websiteId = $this->getpa_website_id();
        $productId = $this->getpa_product_id();
        $this->log('Check available qty for website id = ' . $websiteId . ' and product id = ' . $productId);
        return mage::helper('AdvancedStock/Product_base')->getAvailableQty($productId, $websiteId);
    }
    ...
    protected function getIsSaleable() {
        ...
        if (!$this->getProduct()->getStockItem()->getIsInStock()) {
            $this->log('Product stock status set to out of stock');
            return false;
        }
    ...
    public function getStatus() {
        ...
        if ($this->getpa_is_saleable()) {
            ...
            if ($this->getpa_available_qty() > 0) {
                ...
                return self::kStatusInStock;
                //return mage::helper('SalesOrderPlanning')->__('In stock');
            } else {
                //if has supply date
