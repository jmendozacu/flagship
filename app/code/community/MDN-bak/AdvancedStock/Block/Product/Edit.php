...
class MDN_AdvancedStock_Block_Product_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    public function __construct()
    {
		...
		//display save / update stocks button if allowed
		if (Mage::getSingleton('admin/session')->isAllowed('erp/products/edit'))
		{
			...
			if (Mage::helper('AdvancedStock/MagentoVersionCompatibility')->isQty($this->getProduct()->getTypeId()))
			{
				$this->_addButton(
					...
					array(
						...
						'onclick'   => "window.location.href='".$this->getUrl('AdvancedStock/Products/UpdateStock', array('product_id' => $this->getProduct()->getId()))."'",
						'level'     => -1
					)
				);
			}
        ...
        $this->_addButton(
            'view_product',
            array(
                'label'     => Mage::helper('AdvancedStock')->__('View product'),
    ...
    {
        return $this->getUrl('AdvancedStock/Products/Save');
    }
    
   	public function GetBackUrl()
	{
		return $this->getUrl('AdvancedStock/Products/Grid', array());
	}
