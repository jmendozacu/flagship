...
class Ecommerceguys_Inventorymanager_Model_Shipmanager_Shipment extends Mage_Core_Model_Abstract
{
    ...
    public function completeShipment($orderIncrementId, $shipmentTrackingNumber, $shipmentCarrierCode, $shipmentCarrierTitle)
    {
        ...
        if ($order->canShip()) {
            try {
                ...
                Mage::log(print_r($e, true), null, 'debugging.log');
                throw $e;
            }
        }
    }
    ...
    public function _zencartCustomerShipmentNotify($customername, $customeremail, $orderid, $fedextracking)
    {
        ...
        $emailTemplate = Mage::getModel("core/email_template")->load(7);
        ...
        $emailTemplateVariables['customername'] = (string) $customername;
        $emailTemplateVariables['orderid'] = (string) $orderid;
        $emailTemplateVariables['fedextracking'] = (string) $fedextracking;
        $templateId = 7;
        $sender = array('name' => $senderName, 'email' => $senderEmail);
        
        
        return (Mage::getModel("core/email_template")->sendTransactional($templateId, $sender, $customerEmail, "", $emailTemplateVariables, $store)) ? true : false;
    }
