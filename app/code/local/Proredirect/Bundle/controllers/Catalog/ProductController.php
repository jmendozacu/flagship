...
class Proredirect_Bundle_Catalog_ProductController extends Mage_Catalog_ProductController
{
    /**
    ...
    public function viewAction()
    {
        ...
        $categoryId = (int) $this->getRequest()->getParam('category', false);
        $productId  = (int) $this->getRequest()->getParam('id');
        $parentId = Mage::getModel('bundle/product_type')->getParentIdsByChild($productId);
        $currentWebsiteId = Mage::app()->getWebsite()->getId();
        if(isset($parentId[0]))
           {
                        $item = Mage::getModel('catalog/product')->load($parentId[0]);
