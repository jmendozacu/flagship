...
class Mirasvit_Advr_Model_Report_Abstract extends Mage_Core_Model_Resource_Db_Collection_Abstract
{
    ...
    protected function _joinTable($table, $condition, $callback = false)
    {
        if (!isset($this->_joinedTables[$table]) || $this->_joinedTables[$table] == 2) {
    ...
    public function selectColumns($columns)
    {
        ...
        foreach ($columns as $column) {
            if (isset($this->_columns[$column])) {
                $definition = $this->_columns[$column];
                ...
                if (!isset($this->_selectedColumns[$column])) {
    ...
    protected function _getRangeExpression($range)
    {
        switch ($range) {
            ...
            case '1w':
                ...
                $expr2 = new Zend_Db_Expr('WEEKOFYEAR({{attribute}})');
    ...
    protected function _getTZOffsetTransitions($timezone, $from = null, $to = null)
    {
        ...
        try {
            if ($from == null) {
                $from = new Zend_Date($from, Varien_Date::DATETIME_INTERNAL_FORMAT, Mage::app()->getLocale()->getLocaleCode());
    ...
    public function setFilterData($data)
    {
        foreach ($data->getData() as $column => $value) {
            if (isset($this->_columns[$column])) {
                $this->_columns[$column]->setValue($value);
            }
        }
