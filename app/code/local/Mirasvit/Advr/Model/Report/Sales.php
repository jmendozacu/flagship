...
class Mirasvit_Advr_Model_Report_Sales extends Mirasvit_Advr_Model_Report_Abstract
{
    protected function _construct()
    {
        ...
        $isNewCustomer = 'DATE_FORMAT(sales_order_table.created_at, "%Y-%m-%d") = DATE_FORMAT(customer_entity_table.created_at, "%Y-%m-%d")';
        $shippingTime  = 'unix_timestamp(sales_shipment_table.created_at) - unix_timestamp(sales_order_table.created_at)';
        ...
        )->addColumn('hour_of_day', array(
                'label'             => 'Hour of Day',
                'type'              => 'text',
                'expression_method' => 'getHourOfDayExpression',
                'table'             => 'sales/order',
            )
        )->addColumn('day_of_week', array(
                'label'             => 'Day of Week',
                'type'              => 'text',
                'expression_method' => 'getDayOfWeekExpression',
                'table'             => 'sales/order',
            )
        )->addColumn('payment_method', array(
        ...
        )->addColumn('customer_name', array(
                'label'      => 'Full Name',
                'expression' => 'CONCAT(sales_order_table.customer_firstname, " ", sales_order_table.customer_lastname)',
                'table'      => 'sales/order',
            )
        )->addColumn('category_id', array(
                'label'      => 'Category Id',
                'type'       => 'number',
                'expression' => 'catalog_category_table.entity_id',
                'table'      => 'catalog/category'
            )
        )->addColumn('category_level', array(
                'label'      => false,
        ...
        foreach($attributes as $attrCode => $attrLabel) {
            ...
            $this->addColumn('product_attribute_'.$attrCode, array(
                    'label'        => $attrLabel,
                    'type'         => $type,
                    'options'      => $options,
    ...
    public function joinCategoryName()
    {
        ...
        $conditons = array(
            'catalog_category_name_table.entity_id = catalog_category_table.entity_id',
            'catalog_category_name_table.entity_type_id = '.$category->getTypeId(),
            'catalog_category_name_table.attribute_id = '.$attr->getAttributeId(),
            'catalog_category_name_table.store_id = 0'
        );

        $this->getSelect()->joinLeft(
            array('catalog_category_name_table' => $attr->getBackend()->getTable()),
    ...
    public function joinProductName()
    {
        ...
        $conditions = array(
            'catalog_product_name_table.entity_id = catalog_product_table.entity_id',
            'catalog_product_name_table.entity_type_id = '.$product->getTypeId(),
            'catalog_product_name_table.attribute_id = '.$attr->getAttributeId(),
            'catalog_product_name_table.store_id = 0'
        );

        $this->getSelect()->joinLeft(
            array('catalog_product_name_table' => $attr->getBackend()->getTable()),
    ...
    public function joinProductAttribute($args)
    {
        ...
        if (!isset($this->_joinedTables[$tableName])) {
        ...
        $this->_joinedTables[$tableName] = true;
    ...
    public function onJoinOrderItem($conditions)
    {
        if ($this->getFilterData()->getIncludeChild()) {
            ...
            $conditions = array();
            $conditions[] = 'sales_order_item_table.item_id = IFNULL(sales_order_item_parent_table.parent_item_id, sales_order_item_parent_table.item_id)';
        } else {
    ...
    public function setFilterData($data, $filterByStatus = true)
    {
        ...
        if ($this->_filterData->getFrom()) {
            $conditions[] = $this->getTZDate('sales_order_table.created_at')." >= '".$this->_filterData->getFrom()."'";
        }

        if ($this->_filterData->getTo()) {
            $conditions[] = $this->getTZDate('sales_order_table.created_at')." < '".$this->_filterData->getTo()."'";
        }

        if (count($this->_filterData->getStoreIds())) {
            $conditions[] = 'sales_order_table.store_id IN('.implode(',', $this->_filterData->getStoreIds()).')';
        }
        ...
        if ($filterByStatus) {
            ...
            $conditions[] = '(sales_order_table.status IN('.implode(',', $statuses).') OR sales_order_table.status IS NULL)';
        }
        ...
        foreach ($data->getData() as $column => $value) {
            if (isset($this->_columns[$column])) {
                ...
                $cond = $this->_columns[$column]->getFilter()->getCondition();
