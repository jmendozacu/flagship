...
class Mirasvit_Advr_Block_Adminhtml_Catalog_Attributeset_Detail
    extends Mirasvit_Advr_Block_Adminhtml_Catalog_Abstract
{
    ...
    public function _prepareLayout()
    {
        ...
        $this->setHeaderText(
            Mage::helper('advr')->__('Sales Report for attribute set "%s"',
    ...
    protected function _prepareCollection()
    {
        $collection = Mage::getModel('advr/report_sales')
            ...
            ->selectColumns(array_merge(
                array('product_attribute_set_id'),
                $this->getVisibleColumns())
            )
            ->groupByColumn('period')
            ->groupByColumn('product_attribute_set_id')
            ->addFieldToFilter('product_attribute_set_id', 
                Mage::registry('current_attribute_set')->getId())
            ;

        // echo Mirasvit_SqlFormatter::format($collection->getSelect());
