...
{
    protected $_googleActiveKeyIdx = 0;
    ...
    public function findInMapQuestApi($locations)
    {
        ...
        $get = implode('&', $get);
        $url = 'http://www.mapquestapi.com/geocoding/v1/batch?key=Kmjtd|luua2qu7n9,7a=o5-lzbgq&'.$get.'&outFormat=json';
        $ch = curl_init($url);                                                                      
    ...
    public function findInGoogle($locations)
    {
        ...
        foreach ($locations as $id => $location) {
            ...
            $url = 'https://maps.googleapis.com/maps/api/geocode/json?components=country:'.$country.'|postal_code:'.$code.$key;
            $ch = curl_init($url);                                                                      
            curl_setopt($ch, CURLOPT_REFERER, 'http://www.mapquestapi.com/geocoding/'); 
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            ...
            if (isset($content['status']) && $content['status'] == 'OVER_QUERY_LIMIT') {
                $this->_googleActiveKeyIdx++;
                if ($this->_googleActiveKeyIdx > count($keys) - 1) {
                    Mage::throwException('No valid keys');
                }
            }
