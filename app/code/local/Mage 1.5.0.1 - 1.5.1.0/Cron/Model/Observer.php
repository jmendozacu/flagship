...
class Mage_Cron_Model_Observer
{
     ...
     * 
     *  !!!!!!!!!!!!!!!!!!!! IMPORTANT NOTICE FROM MAISON DU LOGICIEL !!!!!!!!!!!!!!!!!!!!!!!!
     *  Hack core class to avoid cron to execute 2 identical job code during one cron execution
     * 
     *
    ...
    public function dispatch($observer)
    {
        ...
        //#####################
        //Hack maison du logiciel
        //store executed job code
        $executedJobCodes = array();
        ...
        foreach ($schedules->getIterator() as $schedule) {
            ...
            //####################
	        //Hack maison du logiciel
            //prevent from executing 2 times the same job code during one cron execution
            $jobCode = $schedule->getJobCode();
            if (in_array($jobCode, $executedJobCodes))
            	continue;
