...
class MagentoGarden_Image_Adapter_Gd2 extends MagentoGarden_Image_Adapter_Abstract
{
...
//*/
    ...
    public function watermark($watermarkImage, $positionX=0, $positionY=0, $watermarkImageOpacity=30, $repeat=false)
    {
        ...
        $this->_getFileAttributes();
    	$_watermark = call_user_func($this->_getCallback(
            'create',
            $watermarkFileType,
            'Unsupported watermark image format.'
        ), $watermarkImage);
		imagealphablending($_watermark, true);
		$_watermark_width = imagesx($_watermark);
		$_watermark_height = imagesy($_watermark);
		
		$positionX = 0;
		$positionY = 0;
		if (Mage::helper('transparentwatermark')->isUseCustomPosition()) {
			$positionX = Mage::helper('transparentwatermark')->getCustomWatermarkX();
			$positionY = Mage::helper('transparentwatermark')->getCustomWatermarkY();
		} else {
			if ( $this->getWatermarkPosition() == self::POSITION_CENTER ) {
				$positionX = ($this->_imageSrcWidth/2 - imagesx($_watermark)/2);
		        $positionY = ($this->_imageSrcHeight/2 - imagesy($_watermark)/2);
			} elseif( $this->getWatermarkPosition() == self::POSITION_TOP_RIGHT ) {
	            $positionX = ($this->_imageSrcWidth - imagesx($_watermark));
	        } elseif( $this->getWatermarkPosition() == self::POSITION_TOP_LEFT  ) {
	        } elseif( $this->getWatermarkPosition() == self::POSITION_BOTTOM_RIGHT ) {
	            $positionX = ($this->_imageSrcWidth - imagesx($_watermark));
	            $positionY = ($this->_imageSrcHeight - imagesy($_watermark));
	        } elseif( $this->getWatermarkPosition() == self::POSITION_BOTTOM_LEFT ) {
	            $positionY = ($this->_imageSrcHeight - imagesy($_watermark));
        }
		}
		
		imagecopy($this->_imageHandler, $_watermark, $positionX, $positionY, 0, 0, $_watermark_width, $_watermark_height);
		$this->refreshImageDimensions();
		imagedestroy($_watermark);
		return ;
    }
